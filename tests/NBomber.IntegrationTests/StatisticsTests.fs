...
module StepStatsRawData =
    ...
    let ``addResponse should take client response latency if it set`` (isClient: bool, latencyMs: uint32) =
        ...
        let min =
            ...
            |> float
            |> Statistics.Converter.fromMicroSecToMs
    ...
    let ``addResponse should properly calc latency count`` (latencies: uint32 list) =
        ...
        let lessOrEq800 =
            latencies
            |> Seq.filter (fun x -> x <= 800u)
            |> Seq.length

        let more800Less1200 =
            latencies
            |> Seq.filter (fun x -> x > 800u && x < 1200u)
            |> Seq.length

        let moreOrEq1200 =
            latencies
            |> Seq.filter (fun x -> x >= 1200u)
            |> Seq.length
    ...
    let ``addResponse should properly handle OkStats and FailStats`` (latencies: (bool * uint32) list) =
        ...
        |> Seq.iter (fun (isOk, latency) ->
            ...
            let stepResponse =
                { // only stepResponse latency will be included
                  ClientResponse = clientResponse
                  EndTimeMs = Double.MaxValue
        ...
        let okMinStats =
            if okCount > 0 then
                okLatencies |> Seq.map snd |> Seq.min
            else
                0.0

        let okMaxStats =
            if okCount > 0 then
                okLatencies |> Seq.map snd |> Seq.max
            else
                0.0
        ...
        let failLatencies =
            latencies
            |> Seq.filter (fun (isOk, _) -> isOk = false)
        ...
        let failMinStats =
            if failCount > 0 then
                failLatencies |> Seq.map snd |> Seq.min
            else
                0.0

        let failMaxStats =
            if failCount > 0 then
                failLatencies |> Seq.map snd |> Seq.max
            else
                0.0
    ...
    let ``addResponse should properly calc response sizes`` (responseSizes: (bool * uint32) list) =
        ...
        let okResponses =
            responseSizes
            |> Seq.filter (fun (isOk, _) -> isOk)

        let okCount = okResponses |> Seq.length

        let okMinBytes =
            if okCount > 0 then
                okResponses |> Seq.map snd |> Seq.min
            else
                0

        let okMaxBytes =
            if okCount > 0 then
                okResponses |> Seq.map snd |> Seq.max
            else
                0
        ...
        let okAllBytes =
            int64 (
                if okCount > 0 then
                    okResponses |> Seq.map snd |> Seq.sum
                else
                    0
            )
        ...
        let failMinBytes =
            if failCount > 0 then
                failResponses |> Seq.map snd |> Seq.min
            else
                0

        let failMaxBytes =
            if failCount > 0 then
                failResponses |> Seq.map snd |> Seq.max
            else
                0
        ...
        let failAllBytes =
            int64 (
                if failCount > 0 then
                    failResponses |> Seq.map snd |> Seq.sum
                else
                    0
            )
        ...
        let okMin =
            if okCount > 0 then
                data.OkStats.MinBytes
            else
                0

        let okMax =
            if okCount > 0 then
                data.OkStats.MaxBytes
            else
                0

        let failMin =
            if failCount > 0 then
                data.FailStats.MinBytes
            else
                0

        let failMax =
            if failCount > 0 then
                data.FailStats.MaxBytes
            else
                0
...
let ``NodeStats should be calculated properly`` () =
    ...
    |> fun stats ->
        ...
        test
            <@ st1.Ok.Request.Count >= 5
               && st1.Ok.Request.Count <= 10 @>

        test <@ st1.Ok.DataTransfer.MinBytes = 100 @>
        test <@ st1.Fail.Request.Count = 0 @>

        test
            <@ st2.Fail.Request.Count >= 5
               && st2.Fail.Request.Count <= 10 @>
...
let ``status codes should be calculated properly`` () =
    ...
    |> fun stats ->
        ...
        let failStCodes =
            stats.ScenarioStats[0].StepStats[2]
                .Fail
                .StatusCodes
