...
type internal AppError =
    ...
    static member toString (error: ValidationError) =
        ...
        | TargetScenarioNotFound (notFoundScenarios, registeredScenarios) ->
            ...
            sprintf "Target scenarios %s is not found. Available scenarios are %s." notFound available

        | DurationIsWrong scenarioNames ->
            scenarioNames |> String.concatWithCommaAndQuotes
        ...
        | DuplicateStepName stepNames ->
            stepNames |> String.concatWithCommaAndQuotes
            |> sprintf "Step names are not unique: %s."

        | AssertNotFound (i, assertion) ->
            match assertion with
            | Step s -> sprintf "Assertion #'%i' is not found for step: '%s' in scenario: '%s'" i s.StepName s.ScenarioName
