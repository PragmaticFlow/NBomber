...
module StepStats =
    ...
    let create (stepName: string)
               ...
               (clientFactoryName: string option)
               (clientFactoryClientCount: int option)
               (duration: TimeSpan) =
        ...
        { StepName = stepName
          ...
          StepInfo = { Timeout = stepTimeout
                       ClientFactoryName = clientFactoryName |> Option.defaultValue "none"
                       ClientFactoryClientCount = clientFactoryClientCount |> Option.defaultValue 0 } }
