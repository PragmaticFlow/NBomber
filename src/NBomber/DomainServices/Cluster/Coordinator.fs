...
let waitAllAgentsReady (sessionId: string, agents: AgentInfo[]) = task {
    try
        ...
        while not allFinished do
            ...
            let errors = responses |> Array.choose(fun x -> x.Error)
            if errors.Length > 0 then
                errors |> AgentErrors |> Error |> exit
            else
                let stillWorking =
                    responses
                    |> Array.choose(fun x -> x.Data)
                    |> Array.exists(fun x -> x :?> bool)
