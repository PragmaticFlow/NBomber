...
let warmUpScenarios (dep: Dependency, scnRunners: ScenarioRunner[]) =
    scnRunners
    |> Seq.filter(fun x -> x.Scenario.WarmUpDuration.Ticks > 0L)
    |> Seq.iter(fun x ->   Log.Information("warming up scenario: '{0}'", x.Scenario.ScenarioName)
