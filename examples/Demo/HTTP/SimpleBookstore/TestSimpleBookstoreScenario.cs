    ...
    public class TestSimpleBookstoreScenario
    {
        ...
        public ScenarioProps Create()
        {
            ...
            return Scenario
                ...
                {
                    var rundom = random.Next(_settings.ClientsCount - 1);

                    var login = await Step.Run("login", context, async () =>
                    {
                        var rundomUser = userLogins[rundom];

                        var request = Http.CreateRequest("POST", _settings.ServerUrl + "/api/users/login")
                    ...
                    {
                        var request = Http.CreateRequest("GET", _settings.ServerUrl + "/api/books?availableOnly=false")
                            .WithHeader("Accept", "application/json")
                    ...
                    var createOrder = await Step.Run("createOrder", context, async () =>
                    {
                        ...
                        var request = Http.CreateRequest("POST", _settings.ServerUrl + "/api/orders")
                            .WithHeader("Accept", "application/json")
                            .WithHeader("Authorization", $"Bearer {jwt}")
                            .WithJsonBody(order);
                    ...
                    {
                        var request = Http.CreateRequest("POST", _settings.ServerUrl + "/api/users/logout")
                            .WithHeader("Accept", "application/json")
                            .WithHeader("Authorization", $"Bearer {jwt}");
